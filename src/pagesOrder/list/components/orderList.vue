...
const onOrderPay = async (orderId: string) => {
  if (import.meta.env.DEV) {
    const res = await getPayMockAPI({ orderId })
  } else {
    // 微信原生支付逻辑
    const res = await getPayWxPayMiniPayAPI({ orderId })
  ...
  // 成功提示
  const oreder = orderList.value.find((v) => v.id === orderId)
  // 修改订单状态
  oreder!.orderState = OrderState.DaiShouHuo
}
</script>
      ...
      <view class="action">
        ...
        <template v-else>
          <navigator
            ...
            :url="`/pagesOrder/create?orderId=${order.id}`"
            hover-class="none"
          >
            再次购买
          </navigator>
